...
dataset = gdal.Open('../../SourceData/MODIS/TS_CROCUS_Helene_6932_regrid.tiff', gdal.GA_ReadOnly)
...
Corr_NbPixels=np.array(-999999.0*np.zeros((band.YSize,1)))
Ts=np.concatenate((Ts,Corr_NbPixels),axis=1)
Corr_NbPixels=np.array(-999999.0*np.zeros((4,band.XSize+1)))
...
outfile = r'../../SourceData/WorkingFiles/TbSMOSandTsMODIS.nc'
cols = len(X[0,:])
rows = len(Y[:,0])
...
dsout.createVariable('TsMODIS','float64',('y','x'))
dsout.variables['TsMODIS'][:] = np.array(Ts[::-1,:])
dsout.createVariable('TsMODIS-TbSMOS','float64',('y','x'))
dsout.variables['TsMODIS-TbSMOS'][:] = np.array(GapTsTb[::-1,:])
dsout.createVariable('TsMODIS-TsComiso','float64',('y','x'))
dsout.variables['TsMODIS-TsComiso'][:] = np.array(Ts[::-1,:]-Ts_gr[::-1,:])
