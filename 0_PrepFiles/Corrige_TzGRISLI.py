...
#Number of points of the domain
Nx=np.shape(H)[0]
Ny=np.shape(H)[1]
Nz=np.shape(Tz[:,0,0])[0]
...
[[plt.plot(ReducedT[:, i, j], 1/(Nz-1)*np.arange(Nz-1,-1,-1), linewidth=0.1, c='k') for i in np.arange(38,41,1)] for j in np.arange(110,113,1)]
plt.show()
...
for i in np.arange(0,140,1):
    for j in np.arange(0, 140, 1):
        ...
        Zeta[:,i,j]=1/(Nz-1)*np.arange(0,Nz,1)
